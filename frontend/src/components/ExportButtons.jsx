...
function exportToExcel(projects) {
  const rows = projects.map((p) => ({
    ...
    'Formato':           p.formato || '',
    'Cantidad':          p.cantidad,
    'Costo Innovación':  p.costo_inn ?? 0,
    'Costo Placa PAI':   p.costo_placa_pai ?? 0,
    'Costo Lona':        p.costo_lona ?? 0,
    'Otro Proveedor':    p.otro_proveedor_gasto ?? p.ws ?? 0,
    'Total Producción':  p.total_prod ?? 0,
    'Tarifa':            p.tarifa ?? 0,
    'Margen ARS':        p.margen_abs ?? 0,
  ...
  ws['!cols'] = Array(rows.length > 0 ? Object.keys(rows[0]).length : 20).fill({ wch: 18 });
  XLSX.utils.book_append_sheet(wb, ws, 'Proyectos');
  XLSX.writeFile(wb, `american-ads-${new Date().toISOString().split('T')[0]}.xlsx`);
}

function exportToPDF(projects, summary, estado) {
  const html = `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>American ADS · Dashboard</title>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  th { background: rgba(255,106,185,0.08); text-align: left; padding: 8px 10px; font-size: 9px; text-transform: uppercase; letter-spacing: 0.12em; color: #6b7280; font-family: 'Archivo', sans-serif; font-weight: 600; }
  td { padding: 7px 10px; border-bottom: 1px solid rgba(255,255,255,0.04); font-family: 'Archivo', sans-serif; }
  .r { text-align: right; }
  .green { color: #34d399; }
  ...
  .area-badge { display: inline-block; padding: 2px 7px; border-radius: 4px; font-size: 9px; font-weight: 700; }
</style>
</head><body>
<h1>AMERICAN ADS</h1>
...
<thead>
  <tr>
    <th>Área</th><th>Campaña</th><th>Cliente</th><th>OP</th><th>Elemento</th>
    <th class="r">Producción</th><th class="r">Tarifa</th><th class="r">Margen</th><th class="r">Mg%</th>
  </tr>
</thead>
<tbody>
${projects.slice(0, 60).map((p) => `<tr>
  ...
  <td>${p.cliente || '—'}</td>
  <td style="color:#6b7280">${p.op_numero || '—'}</td>
  <td style="color:#6b7280">${p.elemento || p.formato || '—'}</td>
  <td class="r" style="color:#6b7280">$${((p.total_prod || 0) / 1e6).toFixed(1)}M</td>
  <td class="r"><strong>$${((p.tarifa || 0) / 1e6).toFixed(1)}M</strong></td>
  <td class="r green">$${((p.margen_abs || 0) / 1e6).toFixed(1)}M</td>
  <td class="r">${p.margen_pct ? (p.margen_pct * 100).toFixed(1) + '%' : '—'}</td>
</tr>`).join('')}
</tbody>
</table>
</div>
